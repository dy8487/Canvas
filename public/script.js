...
function addMessage(msg, pseudo, date, self) {
	...
	// Scroll 맞춤. 정성일
	var divScroll = document.getElementById('chatEntries');
	var scrollHeight = divScroll.scrollHeight;
	divScroll.scrollTop = scrollHeight;
	
	var slimScroll = $(".slimScrollRail").siblings().css("top", scrollHeight+"px");
...
function setPseudo() {
	if ($(".userNameInput").val() != "") {
		socket.emit('setPseudo', $(".userNameInput").val());
		socket.on('pseudoStatus', function(data) {
			if (data == "ok") {
				$('#modalPseudo').modal('hide');
				$("#alertPseudo").hide();
				pseudo = $(".userNameInput").val();
			} else {
				$("#alertPseudo").slideDown();
			}
		});
